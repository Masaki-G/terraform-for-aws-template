...
module "alb" {
  ...
  acm_certificate_domain_name = "prod.xxx.xxx"
  recode_name                 = "prod"
  zone_id                     = "xxxxxxxxxx"
  security_group_name         = "prod-xxx-xxx"
}
...
module "ecs" {
  ...
  cluster_name     = "prod-example-cluster"
...
module "cicd_backend" {
  ...
  backend_artifact_bucket = "prod-example-s3-artifact"
  source_connection       = "xxxxx"
  backend_repo            = "xxxx/xxxxx"
  backend_branch          = "main"
  backend_ecr_name        = "prod-example-api"
